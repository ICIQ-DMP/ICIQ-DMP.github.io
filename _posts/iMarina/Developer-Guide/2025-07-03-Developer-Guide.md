---
layout: default
title: Developer Guide
permalink: /iMarina/Developer-Guide/
parent: iMarina
has_children: true
---

# iMarina Developer Docs

  - [Guía de instalación](/iMarina/Developer-Guide/How-To-Install)  
    Cómo instalar el proyecto de iMarina para el desarrollo
 
  - [Guía de configuración de PyCharm](/iMarina/Developer-Guide/How-To-Setup-PyCharm)  
    Cómo configurar PyCharm como entorno de desarrollo para el proyecto de iMarina
    
  - [Guía de uso de iMarina](/iMarina/Developer-Guide/How-To-Usage-iMarina)  
    Cómo ejecutar iMarina en local
    
  - [Referencia](/iMarina/Developer-Guide/Reference)  
    Referencia del CLI, de los excels de entrada y el excel de salida de iMarina.

---

